...
TcpConnection::TcpConnection(int fd, EventLoop *pEventLoop)
    : _socket(fd),
      _sio(fd),
      _localAddress(getLocal(fd)),
      _peerAddress(getPeer(fd)),
      _isShutdownWrite(false),
      _pEventLoop(pEventLoop) {}

TcpConnection::~TcpConnection() { shutdownWrite(); }
...
string TcpConnection::Address() const {
  string s;
  s = _localAddress.getIp() + ":" + std::to_string(_localAddress.getPort()) +
      " >> " + _peerAddress.getIp() + ":" +
      std::to_string(_peerAddress.getPort());
  return s;
}
...
void TcpConnection::handleMessageCallBack() {
  if (_messageCB) _messageCB(shared_from_this());
}
